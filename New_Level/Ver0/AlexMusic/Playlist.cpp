...
void Playlist::loadRatings() {
    ...
    qDebug() << "Загрузка рейтингов из:" << ratingsFile;

    savedRatings_.clear();  // Очищаем перед загрузкой

    std::ifstream file(ratingsFile.toStdString());
    if (!file.is_open()) {
        qDebug() << "Файл рейтингов не найден";
        return;
    }
    ...
    while (std::getline(file, line)) {
        ...
        if (std::getline(iss, trackPath, '|') && (iss >> rating)) {
            // === НОРМАЛИЗАЦИЯ ПУТИ ===
            std::string normalized = QDir::cleanPath(
                                         QString::fromStdString(trackPath)
                                         ).toStdString();

            savedRatings_[normalized] = rating;
            qDebug() << "  Загружен рейтинг:"
...
bool Playlist::setTrackRatingByPath(const std::string& path, double rating) {
    if (rating < 0.0 || rating > 5.0) return false;
