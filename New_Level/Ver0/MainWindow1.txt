...
#include <QAudioDevice>      // Qt6 Multimedia
#include <QMediaDevices>     // Qt6 Multimedia
...
// Windows-specific методы и переменные
    ...
    void setupTrayIcon();
    void showTrackNotification(const QString& artist, const QString& title);
    void showCustomTrackNotification(const QString& artist, const QString& title, const QImage& cover);

    QAudioDevice currentAudioDevice;           // текущее устройство вывода
    ...
    std::vector<Track> folderOriginalTracks_;   // Исходный порядок треков из папки
...
namespace {
// true, если первая значимая буква — кириллица
bool startsWithCyrillic(const QString& s) {
    for (const QChar& c : s) {
        if (c.isLetter()) {
            ushort u = c.unicode();
