...
TrackMetadata FastMetadataReader::quickParseID3v2(const QString& filePath) {
    ...
    file.close();
    if (data.size() < 10 || !data.startsWith("ID3")) return metadata;
    ...
    while (pos < data.size() - 10 && pos < tagSize + 10) {
        ...
        pos += 10;
        if (pos + frameSize > data.size()) break;

        if (frameId == "TIT2") {
            metadata.title = QString::fromUtf8(data.mid(pos + 1, frameSize - 1)).trimmed();
        } else if (frameId == "TPE1") {
            metadata.artist = QString::fromUtf8(data.mid(pos + 1, frameSize - 1)).trimmed();
        } else if (frameId == "TALB") {
            metadata.album = QString::fromUtf8(data.mid(pos + 1, frameSize - 1)).trimmed();
        } else if (frameId == "TCON") {
            metadata.genre = QString::fromUtf8(data.mid(pos + 1, frameSize - 1)).trimmed();
        } else if (frameId == "TYER" || frameId == "TDRC") {
            metadata.year = QString::fromUtf8(data.mid(pos + 1, frameSize - 1)).trimmed();
            if (metadata.year.length() > 4) metadata.year = metadata.year.left(4);
        } else if (frameId == "TRCK") {
            metadata.trackNumber = QString::fromUtf8(data.mid(pos + 1, frameSize - 1)).split("/")[0].toInt();
        }
        pos += frameSize;
    }
