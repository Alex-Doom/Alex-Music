...
// Включаем taglib заголовки ТОЛЬКО в .cpp файле
...
QString FastTagReader::decodeTaglibString(const TagLib::String& str) {
    ...
    // Если результат пустой или содержит только пробелы,
    // пробуем Latin1
    if (result.isEmpty() || result.simplified().isEmpty()) {
        result = QString::fromLatin1(str.toCString(true));
    }
