...
void MainWindow::scanFolder(const QString& path) {
    ...
    while (it.hasNext()) {
        ...
        // Загружаем метаданные из MP3
        track.loadMetadata();

        // Если метаданные не загрузились, используем имя файла
    ...
    // Вместо trackList заполняем таблицу
    populateTrackTable();
...
void MainWindow::onSearchTextChanged(const QString& text) {
    // Для QTableWidget используем rowCount()
    for (int i = 0; i < trackTable->rowCount(); ++i) {
        // Получаем элемент в первой колонке (название трека)
        QTableWidgetItem* item = trackTable->item(i, COL_TITLE);
        if (!item) continue;
        ...
        // Проверяем совпадение (ищем по названию и исполнителю)
        bool shouldShow = text.isEmpty();
        if (!shouldShow) {
            // Проверяем название
            shouldShow = original.contains(text, Qt::CaseInsensitive);

            // Если не нашли в названии, проверяем исполнителя
            if (!shouldShow) {
                QTableWidgetItem* artistItem = trackTable->item(i, COL_ARTIST);
                if (artistItem) {
                    shouldShow = artistItem->text().contains(text, Qt::CaseInsensitive);
                }
            }
        }
        ...
        // Подсвечиваем если нужно (только в названии трека)
...
void MainWindow::setupTrackTable() {
    ...
    trackTable->horizontalHeader()->setSectionResizeMode(COL_TITLE, QHeaderView::Stretch);
    trackTable->horizontalHeader()->setSectionResizeMode(COL_ARTIST, QHeaderView::ResizeToContents);
    trackTable->horizontalHeader()->setSectionResizeMode(COL_GENRE, QHeaderView::ResizeToContents);
    trackTable->horizontalHeader()->setSectionResizeMode(COL_ALBUM, QHeaderView::Stretch);
    trackTable->horizontalHeader()->setSectionResizeMode(COL_RATING, QHeaderView::ResizeToContents);
    trackTable->horizontalHeader()->setSectionResizeMode(COL_YEAR, QHeaderView::ResizeToContents);

    // Подключение сигналов
    connect(trackTable, &QTableWidget::cellDoubleClicked,
            this, &MainWindow::onTableDoubleClicked);

    // Установка делегата для рейтинга (если нужно редактирование)
    // trackTable->setItemDelegateForColumn(COL_RATING, new RatingDelegate(this));
}

void MainWindow::populateTrackTable() {
    trackTable->setRowCount(0); // Очищаем таблицу
