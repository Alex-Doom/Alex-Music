...
void MainWindow::scanFolder(const QString& path) {
    ...
    QProgressDialog progressDialog("Сканирование MP3 файлов...", "Отмена", 0, 100, this);
    progressDialog.setWindowModality(Qt::WindowModal);
    progressDialog.setMinimumDuration(500); // Показывать через 500мс
    ...
    qDebug() << "Найдено файлов:" << filePaths.size();

    if (filePaths.isEmpty()) {
        qDebug() << "В папке нет MP3 файлов";
        albumLabel->setText("В папке нет MP3 файлов");
        return;
    }

    progressDialog.setMaximum(filePaths.size());
    ...
    for (const QString& filePath : filePaths) {
        ...
        if (!fileInfo.exists() || fileInfo.size() == 0) {
            processedFiles++;
            progressDialog.setValue(processedFiles);
            QCoreApplication::processEvents();
            continue;
        }

        qDebug() << "Обрабатываем файл:" << fileInfo.fileName();
        ...
        if (track.title().empty()) {
            ...
            qDebug() << "  Использовано имя файла:" << artist << "-" << title;
        } else {
            qDebug() << "  Метаданные:" << track.qArtist() << "-" << track.qTitle();
        }
        ...
        // Обрабатываем события, чтобы UI не зависал
        QCoreApplication::processEvents();
    }

    progressDialog.close();

    qDebug() << "Добавлено треков в плейлист:" << addedFiles;

    if (!progressDialog.wasCanceled() && addedFiles > 0) {
        // Заполняем таблицу
        ...
        // Устанавливаем первый трек текущим
        ...
        qDebug() << "Плейлист загружен, текущий трек:" << playlist.currentIndex();
    }
...
void MainWindow::populateTrackTable() {
    qDebug() << "populateTrackTable: начинаем заполнение таблицы";

    // Отключаем сортировку и сигналы для ускорения
    trackTable->setSortingEnabled(false);
    ...
    qDebug() << "Количество треков для отображения:" << trackCount;
    ...
    // Обновляем заголовки
    trackTable->horizontalHeader()->setVisible(true);
