...
void MainWindow::scanFolder(const QString& path) {
    ...
    while (it.hasNext()) {
        ...
        Track track(filePath.toStdString(),
                    "Unknown Artist",  // временный исполнитель
                    QFileInfo(filePath).baseName().toStdString(),  // временное название
                    "Unknown Album", 0.0);

        // Конструктор автоматически загрузит метаданные из MP3

        playlist.add(track);
        originalTracks_.push_back(track);

        // Используем реальные метаданные для отображения
