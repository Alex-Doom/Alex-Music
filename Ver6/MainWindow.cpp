...
MainWindow::MainWindow(QWidget* parent) : QMainWindow(parent) {
    ...
    albumLabel->setTextInteractionFlags(Qt::TextSelectableByMouse); // ВКЛЮЧАЕМ ВЫДЕЛЕНИЕ
    albumLabel->setCursor(Qt::IBeamCursor); // Меняем курсор на текстовый

    // НАСТРОЙКА artistLabel для выделения текста
    artistLabel = new QLabel();
    artistLabel->setStyleSheet("QLabel { font-size: 14px; color: #ccc; }");
    artistLabel->setWordWrap(true);
    artistLabel->setTextInteractionFlags(Qt::TextSelectableByMouse); // ВКЛЮЧАЕМ ВЫДЕЛЕНИЕ
    artistLabel->setCursor(Qt::IBeamCursor); // Меняем курсор на текстовый
    ...
    leftLayout->addWidget(searchEdit);  // Добавляем поиск перед списком треков
    leftLayout->addWidget(trackList, 1); // Список треков занимает оставшееся место
...
void MainWindow::onMediaStatusChanged(QMediaPlayer::MediaStatus status) {
    if (status == QMediaPlayer::EndOfMedia) {
        // ПРИОРИТЕТ 1: Режим повтора трека
        ...
        // ПРИОРИТЕТ 2: Автопереход к следующему треку
        // Если shuffle включен - используем его логику для автоперехода
        if (playlist.isShuffled()) {
            if (playlist.next()) {
                playCurrentTrack();
            }
        } else {
            // Стандартный автопереход
            onNextClicked();
        }
    }
...
void MainWindow::onTrackListDoubleClicked(QListWidgetItem* item) {
    ...
    if (playlist.setCurrent(row, true)) {  // true = сброс shuffle истории
        playCurrentTrack();
    }
}
...
void MainWindow::onSearchTextChanged(const QString& text) {
    // Сохраняем текущий выбранный трек
    ...
    for (int i = 0; i < trackList->count(); ++i) {
        ...
        // Если строка поиска пустая или текст содержит поисковый запрос
    ...
    // Если был выбран трек и он скрылся - снимаем выделение
